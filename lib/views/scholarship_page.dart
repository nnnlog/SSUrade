...
class _ScholarshipPage extends State<ScholarshipPage> {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: BlocSelector<ScholarshipBloc, ScholarshipState, bool>(
        selector: (state) {
                    ...
                    return const Padding(
                      ...
                      child: Center(
                        child: Column(
                          ...
                          children: [LinearProgressIndicator(), SizedBox(width: 1, height: 15), Text("장학 정보를 불러오고 있어요...")],
                        ),
                      ),
                    );
                  }),
                                      ...
                                      MapEntry(
                                        ...
                                        NumberFormat("###,###,###,###,###,###원").format(
                                          state.scholarshipManager.data
                                              .map((e) => int.parse(e.price.replaceAll(",", "")))
                                              .reduce((value, element) => value + element),
                                        ),
                                      ),
                                    ] +
                                    state.scholarshipManager.data
                                        .map((e) => MapEntry("${e.name}\n(${e.when.displayText})", "${e.process}\n(${e.price}원)"))
                                        .toList(),
                              ),
                            ),
                          ),
