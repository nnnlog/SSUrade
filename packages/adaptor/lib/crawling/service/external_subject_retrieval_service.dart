...
class ExternalSubjectRetrievalService implements ExternalSubjectRetrievalPort {
          ...
          if (includeDetail) {
            ...
            for (var subject in semesterSubjects.subjects.values) {
              final detail = await client
                  .execute("return await ssurade.crawl.getGradeDetail('${yearSemester.year}', '${yearSemester.semester.rawIntegerValue}', '${subject.code}').catch(() => {});")
                  .then((res) {
                    if (res == null) {
            ...
            return semesterSubjects.copyWith(subjects: SplayTreeMap.fromIterable(subjects, key: (subject) => subject.code));
          }
  ...
  Future<Set<YearSemester>> _getCompletedSemesters(WebViewClient client) async {
    return client.execute("return await ssurade.crawl.getGradeSemesterList().catch(() => {});").then((res) {
      ...
      return Set.from(
        res.map((e) {
          return YearSemester(year: int.parse(e['year']), semester: Semester.parse(e['semester']));
        }),
      );
    });
  }
      ...
      data: SplayTreeMap<YearSemester, SemesterSubjects>.fromIterable(
        groupBy(
          data.map((rowData) {
            ...
            final key = YearSemester(year: int.parse(rawKey[0]), semester: Semester.parse("${rawKey[1]}학기"));
          ...
          (() async {
            ...
            return await ParallelWorker(
              ...
              return (semesterSubjects + researchedSubjects).let((it) {
                return SemesterSubjectsManager(data: SplayTreeMap.fromIterable(it, key: (subject) => subject.currentSemester), state: SubjectState.semester);
              });
            });
          })(),
        ]);
